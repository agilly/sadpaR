checkSelectedFolder=function(session, input, output, rootDir, loadedDataset, currentTagging, dur, appPaths){
  ...
  if(! "species.csv" %in% list.files(paste(rootDir(), "metadata", sep="/"))){
    showNotification("No intervals dataset found in metadata directory.", type="error")
    return(F)
  }
...
updateTaggingOnSeqChange=function(session, input, output, currentTagging, loadedDataset, selctid=input$whichCT, selevent=input$sequence){
...
loadDataset=function(session, input, output, rootDir, loadedDataset, currentTagging, dur){
...
saveDataset=function(rootDir, loadedDataset, currentTagging){
