...
retagMultiServer = function(id, merged_dt, species_dt, appLang, savedRetag, rootPath) {
  ...
  refreshBtnColors=function(){
    ...
    if(VERBOSE) print(glue("this status: {this_status} (ctid: {input$ctid_select}, interval: {input$ctid_select})"))
...
retagMultiDemo = function(usePreexisting=F) {
  ...
  server = function(input, output, session) {
    ...
    retag=retagMultiServer("photoModule", reactiveVal(merged_dt), reactiveVal(species_dt), appLang, savedRetag, rootPath = reactiveVal(old_root_dir))
